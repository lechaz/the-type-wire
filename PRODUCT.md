...
- Stack: TanStack Start + Tailwind v4 + shadcn (base-nova preset, Base UI primitives) + Supabase + Gemini (`gemini-3.5-flash-lite` via `@google/genai`) + RapidAPI Real-Time News Data (BASIC plan).
- Hard constraint: RapidAPI BASIC plan is rate-limited to 1 req/sec — category fetches must stay serialized, not parallelized. Two regions double both RapidAPI call volume and Gemini token spend.
