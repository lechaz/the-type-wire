...
## Positioning
The mechanism a plain news aggregator can't copy: real news ingestion (Currents API, RapidAPI, and GDELT, deduped) → AI-tagged decision-maker MBTI (Gemini triage) → personality-reasoned prediction with branching what-if timelines. It's an entertainment/novelty wire service — personality-flavored speculation, not serious forecasting — delivered with total straight-faced wire-bulletin conviction rather than hedging like a disclaimer-heavy tool.
...
- Stack: TanStack Start + Tailwind v4 + shadcn (base-nova preset, Base UI primitives) + Supabase + Gemini (`gemini-3.5-flash-lite` via `@google/genai`) + Currents API (free plan), RapidAPI Real-Time News Data (BASIC plan), and GDELT (free, no key) queried in parallel and deduped — added 2026-07-26 (Currents+RapidAPI) and 2026-07-29 (GDELT) to guarantee 3-5+ articles/category since any single provider alone can come up short or hit quota.
- Hard constraint: Currents free plan caps at 20 req/min, RapidAPI BASIC at 1 req/sec, GDELT self-imposes a slower client-side gate — each provider is throttled independently in `src/server/news/client.ts`. Only 2 of the 3 providers run per fetch; which 2, per region, is decided daily by a health-audit cron (`src/server/news/audit.ts`) rather than a hardcoded pick, added 2026-07-30 after a stale hardcoded exclusion silently starved a category for days. `NEWS_EXCLUDE_PROVIDER` overrides this to pin ingestion away from one provider for debugging.
