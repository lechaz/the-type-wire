...
- Stack: TanStack Start + Tailwind v4 + shadcn (base-nova preset, Base UI primitives) + Supabase + Gemini (`gemini-3.5-flash-lite` via `@google/genai`) + Currents API (free plan) + RapidAPI Real-Time News Data (BASIC plan) queried in parallel and deduped, added 2026-07-26 to guarantee 3-5+ articles/category since either provider alone can come up short or hit quota.
- Hard constraint: Currents free plan caps at 20 req/min, RapidAPI BASIC at 1 req/sec — each provider is throttled independently in `src/server/news/client.ts`. `FORCE_PROVIDER` there lets a dev pin ingestion to a single provider.
